...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO courses (title, description, price, is_active, teacher_id, created_at) 
                VALUES (?, ?, ?, ?, ?, NOW())
            ");
            
            if ($stmt->execute([$title, $description, $price, $is_active, $_SESSION['user_id']])) {
                redirect_with_message('teacher.php', 'تم إنشاء الدورة بنجاح!', 'success');
            } else {
...
<head>
    ...
    <title>إنشاء دورة جديدة - <?php echo htmlspecialchars($user['full_name'] ?? 'معلم'); ?></title>
                ...
                <form method="POST" action="">
                    ...
                    <div style="margin-bottom: 1.5rem;">
                        ...
                        <textarea id="description" name="description" required rows="4"
                                  ...
                                  placeholder="أدخل وصف مفصل للدورة"><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
                    </div>
