...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!$certificate_file) {
                $errors[] = 'فشل في رفع الشهادة الجامعية. يرجى التأكد من أن الملف بصيغة PDF أو صورة.';
            }
        } else {
            ...
            try {
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO users (full_name, username, email, password, phone, id_number, 
                                     user_type, subject_specialization, certificate_file, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, 'teacher', ?, ?, NOW())
                ");
                
                $stmt->execute([
                    $full_name, $username, $email, $hashed_password, $phone, $id_number,
                    $subject_specialization, $certificate_file
                ]);
                ...
                if ($user_id) {
                    ...
                    redirect_with_message('../teacher.php', 'تم إنشاء الحساب بنجاح! مرحباً بك في أكاديمية ARG', 'success');
                } else {
                    throw new Exception('فشل في إنشاء الحساب');
                }
                
            } catch (PDOException $e) {
                error_log("Database error in teacher registration: " . $e->getMessage());
                $errors[] = 'حدث خطأ أثناء إنشاء الحساب. يرجى المحاولة مرة أخرى.';
            } catch (Exception $e) {
                error_log("General error in teacher registration: " . $e->getMessage());
                $errors[] = 'حدث خطأ غير متوقع. يرجى المحاولة مرة أخرى.';
            }
        }
        
    } catch (Exception $e) {
        error_log("Critical error in teacher registration: " . $e->getMessage());
